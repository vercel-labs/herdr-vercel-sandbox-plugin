...
The built-in adapters are Claude Code `2.1.220`, Codex `0.146.0`, and OpenCode `1.18.9`, all lifecycle-verified. A verified adapter needs no flag; `"allowCandidateAgents": true` is only required for custom agents and for a built-in adapter that has been demoted to candidate by an evidence problem. `agentKind` selects one adapter and only that agent is installed in its Sandbox. By default, the plugin reads the `orgId` and `projectId` generated by `vercel link` from the focused worktree's `.vercel/project.json`. Advanced users may set `scope` and `project` together to target a project explicitly. The `projectConfigPath` option points at an existing `.vercel/project.json` outside the worktree.
...
Claude Code `2.1.220` passed the complete capture-harness lifecycle on 2026-08-01 and is promoted by its validated receipt (`verification/receipts/claude-code-2026-08-01`). Codex `0.146.0` and OpenCode `1.18.9` passed the same capture-harness lifecycle on 2026-08-05 and are promoted by their validated receipts (`verification/receipts/codex-2026-08-05`, `verification/receipts/opencode-2026-08-05`). See the [Codex record](docs/codex-live-verification.md), the [OpenCode record](docs/opencode-live-verification.md), and the [Claude Code record](docs/claude-live-verification.md).
