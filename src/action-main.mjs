...
// Runs a child but never lets its stdout reach the terminal before the result
// marker. Captured output is buffered and flushed by emitResult, after the
// marker, preserving the first-line guarantee even when children print (a
// bridge relaying unbounded Vercel CLI output, for example).
function herdrRun(args, options = {}) {
  const result = run(herdr, args, { ...options, capture: true });
  bufferOutput(result.stdout);
  bufferOutput(result.stderr);
  return result;
}
...
function openOnboardingPane(mode, sourcePane, cwd) {
  ...
  herdrRun(["pane", "rename", paneId, account ? "Connect Vercel account" : "Link Vercel project"]);
  herdrRun(["pane", "run", paneId, onboardingCommand(mode, cwd)]);
  emitResult({
    action: actionId,
    ok: true,
...
async function startAgent() {
  ...
  try {
    herdrRun(["pane", "rename", paneId, `${adapter.title} · Vercel Sandbox`]);
    herdrRun(["pane", "run", paneId, bridgeCommand("start", paneId, adapter)]);
  } catch (error) {
  ...
  // The bridge that actually creates the remote Sandbox runs asynchronously in
  // the new pane. This result means "setup was launched", not "Sandbox exists".
  // Orchestrators must poll `info` for remoteCreated/lifecycleState to learn
  // whether creation and setup succeeded.
  emitResult({
    action: "start-agent",
    ok: true,
    phase: "setup-launched",
    paneId,
    sandboxName: latest.panes[paneId].sandboxName,
    agentKind,
    adapterStatus: latest.panes[paneId].adapterStatus,
    lifecycleState: latest.panes[paneId].lifecycleState,
  });
  console.log(`Started ${adapter.title} Sandbox setup in ${paneId}. Poll "info" for remote creation and readiness.`);
}
...
async function runMapped(mode) {
  ...
  if (mode === "reconnect") {
    ...
    herdrRun(["pane", "run", paneId, bridgeCommand("connect", paneId, adapter)]);
